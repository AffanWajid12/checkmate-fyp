...
import { useEnrolledCourses, useStudentAttendance } from '../../../hooks/useCourses';

// ─── Per-course attendance summary card ───────────────────────

const CourseAttendanceCard = ({ course }) => {
    const navigate = useNavigate();
    const { data: records = [], isLoading } = useStudentAttendance(course.id);
    ...
    return (
        <div
            onClick={() => navigate(`/student/courses/${course.id}?tab=attendance`)}
            ...
            <div className="flex items-center gap-3 text-xs text-text-muted">
                {isLoading ? (
                    ...
                    <>
                        ...
                        <span className="ml-auto">{total} session{total !== 1 ? 's' : ''}</span>
                    </>
                )}
            </div>
        </div>
            ...
            <div className="mb-8">
                ...
                <p className="text-sm text-text-secondary mt-1">
                    {isLoading
                        ...
                        : `Showing attendance for ${courses.length} course${courses.length !== 1 ? 's' : ''}.`}
                </p>
            </div>
            ...
            {!isLoading && courses.length === 0 && (
                <div className="flex flex-col items-center justify-center min-h-[40vh] text-center px-6">
                    ...
                    <p className="text-sm text-text-secondary">
                        Enroll in a course to start tracking your attendance.
                    </p>
                </div>
            )}
